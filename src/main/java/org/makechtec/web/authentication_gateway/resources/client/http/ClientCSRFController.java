...
import org.makechtec.web.authentication_gateway.resources.application.http.ApplicationCSRFController;
...
public class ClientCSRFController {
    ...
    private final ClientRateLimitValidator rateLimitValidator;
    ...
    public ClientCSRFController(ClientRateLimitValidator rateLimitValidator, AddressBlackListValidator addressBlackListValidator, HttpServletRequest request, CommonResponseBuilder responseBuilder, CSRFTokenGenerator csrfTokenGenerator) {
    ...
    public ResponseEntity<String> getCSRF(
        ...
        try {
            ...
            CompletableFuture<Boolean> rateLimitFuture = CompletableFuture.supplyAsync(() -> {
                try {
                    return this.rateLimitValidator.hasAttemptsThisClient(applicationIP, clientIP, clientAgent, "csrf");
            ...
            var isAllowed = this.addressBlackListValidator.isAllowedClient(applicationIP)
                    && this.addressBlackListValidator.isAllowedClient(clientIP);
            ...
            CompletableFuture.supplyAsync(() -> {
                try {
                    this.rateLimitValidator.pushAttemptToThisClient(applicationIP, clientIP, clientAgent);
                    return null;
                } catch (SQLException | ClassNotFoundException | InstantiationException | IllegalAccessException e) {
